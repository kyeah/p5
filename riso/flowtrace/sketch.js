...
const createSketch = (parent) => {
  return (p) => {
    ...
    // let goodcolors = ["SEAFOAM", "MELON"];
    let goodcolors = ["TOMATO", "GRASS", "SEAFOAM", "CORAL"];
    ...
    p.preload = () => {
      font = p.loadFont("/riso/fonts/IBMPlexMono-Bold.otf");
    };
    ...
    p.setup = () => {
      ...
      colorLayer = p.createGraphics(width, height);
      colorLayer.pixelDensity(1);
      verts(500);
    ...
    p.draw = () => {
      ...
      for (let x = 0; x < width; x += gran) {
        for (let y = 0; y < height; y += gran) {
          ...
          if (mode === 1) {
            ...
            color.setAlpha(b);
            // p.fill(riso2);
            p.fill(getSrcPixel(x, y, colorLayer));
          } else {
          ...
          // }
          if (b > 24 && fb > 20) {
            let c = charset[Math.round(p.map(fb, 0, 200, 4, charset.length))];
            if (c === "/") {
              p.fill("white");
            }
            p.textFont(font);
      ...
      // p.image(colorLayer, 0, 0);
    };

    const drawSrcLayer = (bgColor) => {
    ...
    const drawColorLayer = () => {
      for (let a = 0; a < 4; a++) {
        console.log(goodcolors[a]);
        let r = goodcolors
          ? RISOCOLORS.find((c) => c.name === goodcolors[a])
          : p.random(RISOCOLORS);
        console.log(r);
        colorLayer.stroke(r.color);
        colorLayer.strokeWeight(4);
        // colorLayer.stroke("");

        for (let i = 0; i < 3000; i++) {
          let x = Math.floor(p.random(numColumns));
          let y = Math.floor(p.random(numRows));
          drawCurve(x, y, 50, width * 0.0001);
        }

        colorLayer.loadPixels();
      }
    };

    // Draw a curve starting at (x, y)
    const drawCurve = (x, y, numSteps, stepLength) => {
      colorLayer.beginShape();

      for (let step = 0; step < numSteps; step++) {
        colorLayer.curveVertex(x * tileW, y * tileH);

        // const xOffset = x - leftX;
        // const yOffset = y - topY;

        // const colIdx = Math.floor(x / resolution);
        // const rowIdx = Math.floor(x / resolution);

        try {
          let gridAngle = grid[Math.floor(x)][Math.floor(y)];
